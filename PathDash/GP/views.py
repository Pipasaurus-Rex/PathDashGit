...
from .models import Cost, Gptests, Location
...
def costs_bubble(request):
    ...
    loc_exp=[row[2] for row in locations if row[0] == loca][0]
    data={}
...
def r_by_location(request):
    ...
    data2=json.dumps(data)
    cons=json.dumps(con_codes)
    return render(request, 'GP/level_by_location.html', {'start':start, 'end':end, 'data':data2, 'loc_list':loc_list, 'location':location, 'loc_exp':loc_exp, 'con_codes':cons})	
...
def boxplots_set_by_loc(request):
    ...
    data2=json.dumps(data)
    loc_exps=json.dumps(loc_exp)
    return render(request, 'GP/boxplots_set_by_loc.html', {'data':data2, 'num_locs':num_locs, 'start':start, 'end':end, 'loc_list':loc_list, 'loca':location, 'loc_exp':loc_exps})
